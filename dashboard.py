...
if analyze or stock_input:
    ...
    if len(data) == 0:
        ...
        candle.update_layout(template="plotly_dark", hovermode="x unified", height=400)
        ...
        signal_fig.add_trace(go.Scatter(x=data.index, y=data['Close'], mode='lines', name='Price', line=dict(color='white', width=1.5)))
        signal_fig.add_trace(go.Scatter(x=data.index, y=data['MA20'], mode='lines', name='MA20', line=dict(color='royalblue', dash='dash')))
        signal_fig.add_trace(go.Scatter(x=data.index, y=data['MA50'], mode='lines', name='MA50', line=dict(color='orange', dash='dash')))
        signal_fig.add_trace(go.Scatter(x=data.index, y=data['Buy'], mode='markers', name='BUY 🟢', marker=dict(color='lime', size=10, symbol='triangle-up')))
        signal_fig.add_trace(go.Scatter(x=data.index, y=data['Sell'], mode='markers', name='SELL 🔴', marker=dict(color='red', size=10, symbol='triangle-down')))
        signal_fig.update_layout(template="plotly_dark", hovermode="x unified", height=400)
